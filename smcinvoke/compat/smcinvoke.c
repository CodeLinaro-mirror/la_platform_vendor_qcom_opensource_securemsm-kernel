		...
		unsigned long arg)
{
	...
	/* In caase on nr_args is zero, allocating the one buffer so that args_buf
	   points to valid buffer. There is no need to copy anything to buffer.
	*/
	nr_args_cnt = ((nr_args > 0) ? nr_args : (nr_args + 1));
